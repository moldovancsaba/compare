...
## Application Layers
- UI: Next.js App Router pages and React components in `src/app` and `src/components`, including an adapter-aware domain selector, accountless watch collection profile panel with gap/overlap analysis, editorial result hierarchy, decision scan panel, and shareable saved comparison pages at `/compare/[slug]`.
