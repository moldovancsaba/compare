...
## Application Layers
- UI: Next.js App Router pages and React components in `src/app` and `src/components`, including an adapter-aware domain selector, accountless watch collection profile panel with gap/overlap analysis and balance scoring, single-watch should-I-buy guidance with five-year ownership simulation and market positioning, collection-aware upgrade-path guidance, editorial result hierarchy, decision scan panel, and shareable saved comparison pages at `/compare/[slug]`.
