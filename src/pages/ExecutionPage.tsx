...
import {
  Rocket, Mail, Globe, DollarSign, Users, Target, BarChart3,
  Megaphone, ArrowRight, Play, Pause, Plus, TrendingUp, Filter,
  Zap, MousePointerClick, Layers,
} from "lucide-react";
...
const campaigns = [
  ...
  { name: "Content-Led Inbound", type: "Inbound", status: "active" as const, sent: 0, opened: 0, replied: 0, meetings: 0, budget: "$1,800", channel: "Blog + SEO", visitors: 12400, leads: 342 },
  { name: "LinkedIn Thought Leadership", type: "Inbound", status: "active" as const, sent: 0, opened: 0, replied: 0, meetings: 0, budget: "$500", channel: "LinkedIn", visitors: 45000, leads: 89 },
  { name: "Google Ads — AI GTM Keywords", type: "Paid", status: "active" as const, sent: 0, opened: 0, replied: 0, meetings: 0, budget: "$8,500", channel: "Google Ads", clicks: 3200, conversions: 128 },
  { name: "LinkedIn Ads — Enterprise", type: "Paid", status: "paused" as const, sent: 0, opened: 0, replied: 0, meetings: 0, budget: "$5,200", channel: "LinkedIn Ads", clicks: 1800, conversions: 45 },
  { name: "Retargeting — Website Visitors", type: "Paid", status: "active" as const, sent: 0, opened: 0, replied: 0, meetings: 0, budget: "$2,100", channel: "Multi-channel", clicks: 890, conversions: 67 },
];
...
const leadDatabase = [
  ...
  { name: "Elena Rodriguez", company: "ScaleUp AI", title: "Head of Growth", score: 78, status: "warm" as const, source: "LinkedIn" },
  { name: "James Park", company: "CloudNext", title: "CRO", score: 72, status: "warm" as const, source: "Google Ads" },
  { name: "Lisa Thompson", company: "RevOps Co", title: "Director of Marketing", score: 65, status: "warm" as const, source: "Content" },
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold font-display text-foreground">Acquisition & Execution</h1>
          <p className="text-sm text-muted-foreground mt-1">Manage campaigns, generate leads, and optimize your funnel.</p>
        </div>
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Active Campaigns", value: "5", icon: Rocket, sub: "1 paused" },
          { label: "Total Leads", value: "3,420", icon: Users, sub: "+342 this week" },
          { label: "Ad Spend", value: "$22.3K", icon: DollarSign, sub: "This month" },
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-lg bg-accent/10 flex items-center justify-center">
                    {camp.type === "Outbound" ? <Mail className="w-[18px] h-[18px] text-accent" /> :
                     camp.type === "Inbound" ? <Globe className="w-[18px] h-[18px] text-accent" /> :
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {camp.type === "Outbound" ? (
                  ...
                  <>
                    ...
                    <div className="bg-muted/50 rounded-md px-3 py-2"><p className="text-[10px] text-muted-foreground">Conv. Rate</p><p className="text-sm font-semibold text-foreground">{(camp as any).visitors && (camp as any).leads ? (((camp as any).leads / (camp as any).visitors) * 100).toFixed(1) + "%" : "—"}</p></div>
                ...
                <tbody className="divide-y divide-border">
                  {leadDatabase.map((lead, i) => (
                    <tr key={i} className="hover:bg-muted/20 transition-colors">
                      ...
                      <td className="px-4 py-3 text-center"><Button variant="ghost" size="sm" className="text-[10px] h-6">View <ArrowRight className="w-3 h-3 ml-1" /></Button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
                  ...
                  <div className="flex-1">
                    <div className="h-8 rounded-md bg-muted/50 overflow-hidden">
                      <motion.div
                        ...
                        className="h-full bg-accent/20 flex items-center px-3"
                      >
        ...
        <TabsContent value="optimization" className="space-y-4 mt-4">
          {[
            ...
            { title: "Lead Scoring Model Update", status: "Scheduled", metric: "AI recommends adding intent signals", icon: Target, action: "Review" },
            { title: "Funnel Drop-off Analysis", status: "Complete", metric: "MQL→SQL stage losing 64% — nurture gap", icon: Layers, action: "See Plan" },
          ].map((opt, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              className="bg-card border border-border rounded-lg p-5 shadow-card flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-lg bg-accent/10 flex items-center justify-center">
                  <opt.icon className="w-[18px] h-[18px] text-accent" />
                </div>
                <div>
