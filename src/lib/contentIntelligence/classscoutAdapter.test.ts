...
describe("RangeScout adapter resolver", () => {
  it("builds a provider draft with recurring programs and time tags", () => {
    const result = resolveRangeScoutDraft({
      ...
      addressRaw: "34 Example Street, Budapest, HU",
      activityTypesRaw: ["Rifle"],
      ageRangesRaw: ["3–5", "6–8"],
      descriptionFacts: ["Weekly rifle fundamentals at a Budapest indoor range."],
      contactFacts: {
  ...
  it("builds a meetup draft and flags missing image uploads", () => {
    const result = resolveRangeScoutDraft({
      ...
      boroughRaw: "Hungary",
      neighborhoodRaw: "Budapest",
      descriptionFacts: ["A weekly sport shooting meetup focused on range practice and match prep."],
      contactFacts: {
  ...
  it("does not infer afternoon tags from morning schedules", () => {
    const result = resolveRangeScoutDraft({
      ...
      boroughRaw: "Hungary",
      neighborhoodRaw: "Budapest",
      descriptionFacts: ["Saturday morning marksmanship class in Budapest."],
      contactFacts: {
